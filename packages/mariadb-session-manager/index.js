...
module.exports = function (session) {
  ...
  class PGStore extends Store {
     ...
     */
    pruneSessions (fn) {
      this.query('DELETE FROM ' + this.quotedTable() + ' WHERE expire < TIMESTAMP(?)', [currentTimestamp()], err => {
     ...
     * @param {any} [updatedSid]
     * @returns {Promise<PGStoreQueryResult|undefined>}
     * @access private
     */
    async _asyncQuery (query, params, noTableCreation, updatedSid) {
     ...
     * @access private
     */
    query (query, params, fn, updatedSid, noTableCreation) {
      /** @type {any[]} */
      let resolvedParams;
      ...
      const result = this._asyncQuery(query, resolvedParams, noTableCreation, updatedSid);
    ...
    get (sid, fn) {
      ...
      this.query('SELECT sess FROM ' + this.quotedTable() + ' WHERE sid = ? AND expire >= FROM_UNIXTIME(?)', [sid, currentTimestamp()], (err, data) => {
    ...
    set (sid, sess, fn) {
      ...
      const query = 'CALL PUT_SESSION(?, ?, FROM_UNIXTIME(?));';
    ...
    destroy (sid, fn) {
      ...
      this.query(
        'DELETE FROM ' + this.quotedTable() + ' WHERE sid = ?',
        [sid],
        err => { fn && fn(err); }
      );
    }
    ...
    touch (sid, sess, fn) {
      ...
      this.query(
        'UPDATE ' + this.quotedTable() + ' SET expire = FROM_UNIXTIME(?) WHERE sid = ?',
        [expireTime, sid],
        err => { fn && fn(err); },
        sid
      );
